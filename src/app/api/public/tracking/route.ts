...
const TRACKING_SELECT = `
  id, code, customer_name, country, carrier, status, created_at,
  ...
  customer:customers!shipments_customer_id_fkey(first_name, last_name, phones)
`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    await enforcePublicTrackingRateLimit(
      request.headers,
  ...
  const { data, error } = await admin
    ...
    .eq("public_tracking_token_hash", createHash("sha256").update(token).digest("hex"))
    .is("public_tracking_revoked_at", null)
    .gt("public_tracking_expires_at", new Date().toISOString())
    .maybeSingle();
  ...
  const row = data as (PublicTrackingLookupRow & { id: string }) | null;
