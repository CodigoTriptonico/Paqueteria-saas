...
export async function setDefaultWarehouseAction(
  ...
  try {
    ...
    if (!warehouse.is_active) {
      return fail("Solo puedes marcar bodegas activas como principal");
    }

    if (warehouse.is_default) {
      return ok(warehouse as WarehouseRow);
    }
    ...
    if (setError || !updated) {
      return fail(setError?.message || "No se pudo cambiar la bodega principal");
    }

    return ok(updated as WarehouseRow);
  } catch (error) {
    return fail(actionErrorMessage(error));
  }
}
...
export async function copyWarehouseCatalogAction(
  ...
  try {
    ...
    const copied = await copyWarehouseStockRows(
      supabase,
      session.organizationId,
      sourceWarehouseId,
      targetWarehouseId,
