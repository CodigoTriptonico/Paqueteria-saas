      ...
      <div className="mx-auto w-full max-w-3xl">
        ...
        <section className="rounded-2xl border border-black bg-surface-panel p-4 shadow-[0_18px_50px_rgba(0,0,0,0.22)] sm:p-5">
          <p className="max-w-xl text-sm font-bold leading-relaxed text-slate-300">
            Escanea el QR de tu recibo. El enlace seguro es aleatorio, expira y puede revocarse.
          </p>
          <form onSubmit={submit} className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            <label className="grid gap-1.5">
              <span className="text-[11px] font-black uppercase tracking-wide text-slate-400">Token seguro</span>
              <input value={token} onChange={(event) => setToken(event.target.value.trim())} autoComplete="off" placeholder="Abre el enlace del QR" className="h-12 rounded-xl border border-black bg-surface-inset px-3 text-sm font-black outline-none transition focus:border-emerald-400 focus:ring-2 focus:ring-emerald-400/20" />
            </label>
        ...
        {shipment ? <TrackingResult shipment={shipment} /> : <section className="mt-5 rounded-2xl border border-black/70 bg-surface-card/45 p-4 text-sm font-bold text-slate-400 sm:p-5"><span className="flex items-center gap-2 text-slate-300"><CircleHelp className="h-4 w-4 text-emerald-300" />¿No tienes el enlace seguro?</span><p className="mt-2 leading-relaxed">Pídelo a la persona que realizó el envío o escanea el QR del recibo.</p></section>}
      </div>
    </main>
  );
}
...
function TrackingResult({ shipment }: { shipment: PublicTrackingShipment }) {
  return <section className="mt-5 space-y-4">
    <header className="overflow-hidden rounded-2xl border border-black bg-surface-card shadow-[0_12px_28px_rgba(0,0,0,0.17)]">
      ...
      <div className="grid gap-3 p-4 sm:grid-cols-2 sm:p-5"><Detail icon={Truck} label="Servicio" value={shipment.carrier || "Envío estándar"} /><Detail icon={MapPin} label="Destino" value={shipment.recipient.destination || shipment.country} /></div>
    </header>
    ...
    <section className="grid gap-4 sm:grid-cols-2"><Person title="Remitente verificado" name={shipment.sender.name} address="" /><Person title="Destino" name={shipment.recipient.name || "Destinatario"} address={shipment.recipient.destination} /></section>

    <section className="rounded-2xl border border-black bg-surface-card p-4 sm:p-5"><h3 className="text-sm font-black text-slate-100">Cajas</h3><div className="mt-3 grid gap-2">{shipment.boxes.map((box) => <div key={`${box.label}-${box.quantity}`} className="flex items-center justify-between gap-3 rounded-xl border border-black bg-surface-inset px-3 py-2.5 text-sm"><span className="font-bold text-slate-200">{box.label}</span><b className="shrink-0 text-emerald-300">×{box.quantity}</b></div>)}{!shipment.boxes.length ? <p className="text-sm font-bold text-slate-500">Sin detalle de cajas.</p> : null}</div></section>
