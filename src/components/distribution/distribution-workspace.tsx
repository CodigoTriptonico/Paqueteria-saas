...
import { Panel, StatCard, inputClass, primaryButtonClass, secondaryButtonClass } from "@/components/ui-blocks";
...
function CreatePartner({ onRun, onDone, isPending }: { onRun: (task: () => Promise<{ ok: boolean; error?: string }>, success: string) => void; onDone: () => void; isPending: boolean }) {
  ...
  return <div className="mt-4 grid gap-2 border-t border-black pt-4 md:grid-cols-2 xl:grid-cols-5">
    ...
    <input className={fieldClass} value={fullName} onChange={(event) => setFullName(uppercasePersonNameInput(event.target.value))} placeholder="Responsable" />
...
function ProductsTab({ partner, catalog, isPending, onRun }: { partner: DistributionPartner; catalog: DistributionCatalogItem[]; isPending: boolean; onRun: (task: () => Promise<{ ok: boolean; error?: string }>, success: string) => void }) {
  ...
  return <Panel title="Productos y tarifa interna" hideHeader contentClassName="space-y-3 p-0">
    ...
    <p className="text-xs font-bold text-slate-400">El distribuidor decide el público. Tú solo defines cuánto te paga por cada venta.</p>
...
function CaptorAccessTab({ partner, captors, isPending, onRun }: { partner: DistributionPartner; captors: DistributionCaptor[]; isPending: boolean; onRun: (task: () => Promise<{ ok: boolean; error?: string }>, success: string) => void }) {
  ...
  return <div className="space-y-4"><Panel title="Captador responsable" hideHeader contentClassName="p-3"><div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_auto]"><select className={fieldClass} value={captorId} onChange={(event) => setCaptorId(event.target.value)}><option value="">Sin asignar</option>{captors.map((captor) => <option key={captor.id} value={captor.id}>{captor.name}</option>)}</select><button disabled={isPending || !captorId || captorId === partner.acquisitionOwnerId} onClick={() => onRun(() => assignDistributionPartnerCaptorAction({ partnerId: partner.id, captorId, reason: "Reasignacion desde matriz" }), "Captador reasignado. Las ventas anteriores conservan su historial.")} className={secondaryButtonClass}>Guardar captador</button></div><p className="mt-2 text-xs font-bold text-slate-400">La reasignacion solo afecta ventas futuras. El historial conserva al captador que tenia cada venta.</p></Panel><AccessTab partner={partner} isPending={isPending} onRun={onRun} /></div>;
}
...
function AccessTab({ partner, isPending, onRun }: { partner: DistributionPartner; isPending: boolean; onRun: (task: () => Promise<{ ok: boolean; error?: string }>, success: string) => void }) {
  ...
  const [password, setPassword] = useState("");
  return <Panel title="Acceso y control" hideHeader contentClassName="space-y-4 p-0"><div className="grid gap-2 md:grid-cols-2"><input className={fieldClass} value={name} onChange={(event) => setName(event.target.value)} placeholder="Empresa" /><input className={fieldClass} value={ownerName} onChange={(event) => setOwnerName(uppercasePersonNameInput(event.target.value))} placeholder="Responsable" /><input className={fieldClass} type="email" value={ownerEmail} onChange={(event) => setOwnerEmail(event.target.value)} placeholder="Correo de acceso" /><button disabled={isPending} onClick={() => onRun(() => updateDistributionPartnerAction({ partnerId: partner.id, name, ownerName, ownerEmail }), "Datos del distribuidor actualizados.")} className={secondaryButtonClass}>Guardar datos</button></div><div className="grid gap-2 rounded-lg border border-black bg-surface-inset p-3 md:grid-cols-[minmax(0,1fr)_auto]"><label><span className="mb-1 block text-xs font-black uppercase text-slate-400">Límite de crédito</span><input className={fieldClass} inputMode="decimal" value={creditLimit} onChange={(event) => setCreditLimit(event.target.value)} /></label><button disabled={isPending} onClick={() => onRun(() => updateDistributionCreditLimitAction({ partnerId: partner.id, creditLimit: Number(creditLimit) }), "Límite de crédito actualizado.")} className={`${secondaryButtonClass} self-end`}>Guardar límite</button></div><div className="grid gap-2 rounded-lg border border-black bg-surface-inset p-3 md:grid-cols-[minmax(0,1fr)_auto]"><label><span className="mb-1 block text-xs font-black uppercase text-slate-400">Nueva contraseña</span><input className={fieldClass} type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Mínimo 6 caracteres" /></label><button disabled={isPending || password.length < 6} onClick={() => onRun(() => resetDistributionPartnerPasswordAction({ partnerId: partner.id, password }), "Contraseña de acceso actualizada.")} className={`${secondaryButtonClass} self-end`}><KeyRound className="h-4 w-4" /> Restablecer</button></div><div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-black bg-surface-inset p-3"><span className="text-sm font-bold text-slate-300">No se elimina una cuenta con historial. Pausarla bloquea su acceso y nuevas ventas.</span><button disabled={isPending} onClick={() => onRun(() => setDistributionPartnerStatusAction({ partnerId: partner.id, isActive: !partner.isActive }), partner.isActive ? "Distribuidor pausado y acceso bloqueado." : "Distribuidor reactivado.")} className={partner.isActive ? secondaryButtonClass : primaryButtonClass}>{partner.isActive ? <PauseCircle className="h-4 w-4" /> : <PlayCircle className="h-4 w-4" />}{partner.isActive ? "Pausar distribuidor" : "Reactivar distribuidor"}</button></div></Panel>;
}
