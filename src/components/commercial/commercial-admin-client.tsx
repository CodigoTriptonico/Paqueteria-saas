...
function PriceEditor({ data, entity, priceKind }: { data: CommercialAdminData; entity: CommercialEntity; priceKind: "public" | "internal" }) {
  ...
  function save(line: CommercialAdminData["catalog"][number], level: "group" | "entity") {
    ...
    startTransition(async () => {
      const result = await saveCommercialOverrideAction({ audience: entity.type, entityId: level === "entity" ? entity.id : null, destinationCode: line.destinationCode, productCode: line.productCode, priceKind, serviceConcept: "international_shipping", amountCents });
  ...
  return <div className="space-y-3">
    ...
    <p className="text-xs font-bold text-slate-400">Prioridad efectiva: entidad → grupo de {entity.type === "agency" ? "agencias" : "vendedores"} → país.</p>
    <div className="grid gap-2">{lines.map((line) => {
      ...
      return <article key={line.productCode} className="rounded-xl border border-black bg-surface-list-row p-3">
        ...
        <div className="mt-3 grid gap-2 lg:grid-cols-2">
          {(["group", "entity"] as const).map((level) => { const override = level === "group" ? resolved.group : resolved.own; const key = `${level}:${line.destinationCode}:${line.productCode}:${priceKind}`; return <div key={level} className="rounded-lg border border-black bg-surface-inset p-2.5"><div className="mb-2 flex items-center justify-between gap-2"><span className="text-[10px] font-black uppercase text-slate-400">{level === "group" ? "Todas" : "Esta entidad"}</span>{override ? <button className="inline-flex items-center gap-1 text-[10px] font-black text-amber-200" onClick={() => restore(override)} disabled={pending}><RotateCcw className="h-3 w-3" /> Volver a heredar</button> : <span className="text-[10px] font-bold text-slate-600">Sin excepción</span>}</div><div className="flex gap-2"><input className={`${inputClass} min-w-0 flex-1`} inputMode="decimal" placeholder={override ? String(override.amountCents / 100) : String(baseAmount / 100)} value={drafts[key] || ""} onChange={(event) => setDrafts((current) => ({ ...current, [key]: event.target.value }))} /><button className={secondaryButtonClass} onClick={() => save(line, level)} disabled={pending || !drafts[key]}>Guardar</button></div></div>; })}
        </div>
      </article>;
    })}</div>
  </div>;
...
function EntityDetail({ data, entity }: { data: CommercialAdminData; entity: CommercialEntity }) {
  ...
  return <div className="space-y-3">
    <header className="rounded-xl border border-black bg-surface-card p-4 shadow-[0_8px_24px_rgba(0,0,0,0.2)]"><div className="flex flex-wrap items-start gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-lg border border-black bg-surface-inset text-emerald-300">{entity.type==="agency"?<Building2 className="h-5 w-5"/>:<Store className="h-5 w-5"/>}</span><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-wide text-emerald-300">{entity.type==="agency"?entity.code:"Vendedor"}</p><h2 className="truncate text-2xl font-black text-slate-50">{entity.name}</h2><p className="text-sm font-bold text-slate-400">{entity.type==="agency"?"Configura el perfil. Los saldos y pagos permanecen fuera de esta página.":"Configura reglas de venta. Las ventas reales permanecen en Venta e Historial."}</p></div><span className="rounded-full border border-black bg-surface-inset px-3 py-1 text-xs font-black text-slate-300">{entity.status}</span></div></header>
    ...
    {tab==="internal"&&entity.type==="agency"?<Panel title="Tarifas internas de la matriz"><p className="mb-3 text-xs font-bold text-slate-400">Solo configura cuánto debe la agencia a la matriz. No representa saldos ni pagos.</p><PriceEditor data={data} entity={entity} priceKind="internal"/></Panel>:null}
    ...
    {tab==="users"?<Panel title="Usuarios y permisos"><div className="grid gap-2"><p className="rounded-lg border border-black bg-surface-list-row p-3 text-sm font-black text-slate-200"><Users className="mr-2 inline h-4 w-4 text-emerald-300"/>{entity.userCount} usuario{entity.userCount===1?"":"s"} asociado{entity.userCount===1?"":"s"}</p><p className="text-xs font-bold text-slate-500">Los roles y permisos se administran con el módulo existente de equipo. Aquí solo se muestra el contexto comercial.</p></div></Panel>:null}
...
export function CommercialAdminClient({ initialData, initialAudience="agency", selectedEntityId }: { initialData: CommercialAdminData; initialAudience?: AudienceTab; selectedEntityId?: string }) {
  const [audience,setAudience]=useState<AudienceTab>(initialAudience); const [query,setQuery]=useState(""); const [country,setCountry]=useState(""); const [status,setStatus]=useState("");
  ...
  if(selected) return <div className="mx-auto w-full max-w-[1500px] space-y-3 p-3 sm:p-5"><Link href={selected.type==="agency"?"/agencias":"/vendedores"} className={`${secondaryButtonClass} w-fit`}>Volver a Vendedores y Agencias</Link><EntityDetail data={initialData} entity={selected}/></div>;
  return <div className="mx-auto w-full max-w-[1500px] space-y-4 p-3 sm:p-5"><header className="rounded-xl border border-black bg-surface-card p-4 shadow-[0_8px_24px_rgba(0,0,0,0.22)]"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border border-black bg-surface-inset text-emerald-300"><Settings2 className="h-5 w-5"/></span><div><p className="text-xs font-black uppercase tracking-wide text-emerald-300">Administración comercial</p><h1 className="text-2xl font-black text-slate-50">Vendedores y Agencias</h1><p className="mt-1 text-sm font-bold text-slate-400">Perfiles, rutas, reglas y precios. Las ventas, cobros y saldos permanecen en sus módulos.</p></div></div></header><AppTabs tabs={audienceTabs} value={audience} onChange={setAudience} ariaLabel="Vendedores y agencias"/><Panel title={audience==="agency"?"Agencias":"Vendedores"} action={<span className="text-xs font-black text-slate-400">{entities.length} resultado{entities.length===1?"":"s"}</span>}><div className="mb-3 grid gap-2 sm:grid-cols-[minmax(0,1fr)_12rem_12rem]"><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500"/><input className={`${inputClass} w-full pl-9`} value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Buscar por nombre, código o correo"/></label><select className={inputClass} value={country} onChange={(event)=>setCountry(event.target.value)}><option value="">Todos los países</option>{initialData.countries.map((item)=><option key={item.code} value={item.code}>{item.name}</option>)}</select><select className={inputClass} value={status} onChange={(event)=>setStatus(event.target.value)}><option value="">Todos los estados</option><option value="active">Activo</option><option value="inactive">Inactivo</option><option value="temporarily_suspended">Suspendido</option></select></div><div className="grid gap-2">{entities.map((entity)=><article key={entity.id} className="grid gap-3 rounded-xl border border-black bg-surface-list-row p-3 md:grid-cols-[minmax(0,1fr)_9rem_12rem_9rem] md:items-center"><div className="min-w-0"><p className="truncate font-black text-slate-100">{entity.name}</p><p className="truncate text-xs font-bold text-slate-500">{entity.type==="agency"?`${entity.code} · ${entity.userCount} usuarios`:entity.email}</p></div><span className="text-xs font-black text-slate-300">{entity.profile.countryCode||"Sin país"}</span><span className="truncate text-xs font-bold text-slate-400">{entity.type==="agency"?entity.routeName:entity.profile.zone||"Sin zona"}</span><Link href={entity.type==="agency"?`/agencias/${entity.id}`:`/vendedores/${entity.id}`} className={`${secondaryButtonClass} w-full`}>Configurar</Link></article>)}{!entities.length?<p className="rounded-lg border border-dashed border-slate-700 p-8 text-center text-sm font-bold text-slate-400">No hay resultados con estos filtros.</p>:null}</div></Panel></div>;
}
