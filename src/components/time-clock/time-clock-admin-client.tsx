      ...
      <div className="space-y-4 p-4 pb-8 sm:p-5 sm:pb-10">
        ...
        {tab === "reports" ? (
          <div className="space-y-4">
            ...
            <section className="overflow-hidden rounded-xl border border-black bg-surface-card">
              <div className="grid grid-cols-[minmax(0,1fr)_auto_auto_auto] gap-3 border-b border-black bg-surface-card-header px-4 py-2 text-[10px] font-black uppercase tracking-wide text-slate-500"><span>Empleado / fecha</span><span>Regular</span><span>Extra</span><span>Total</span></div>
              <div className="flex flex-col gap-2 p-2">
                {reportRows.map((row) => <div key={`${row.employeeId}-${row.date}`} className={`${listRowBaseClass} grid grid-cols-[minmax(0,1fr)_auto_auto_auto] items-center gap-3 px-4 py-2.5 text-sm`}><div className="min-w-0"><p className="truncate font-black">{row.employeeName}</p><p className="text-[10px] font-bold text-slate-500">{row.employeeId} · {row.date}</p></div><span className="font-black tabular-nums text-slate-300">{row.regularHours.toFixed(2)}</span><span className="font-black tabular-nums text-amber-300">{row.overtimeHours.toFixed(2)}</span><span className="font-black tabular-nums text-emerald-300">{row.totalHours.toFixed(2)}</span></div>)}
